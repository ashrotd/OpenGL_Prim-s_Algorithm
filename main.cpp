    ...
    void display() {
        ...
        for (const auto& edge : edges) {
            ...
            std::string edgeInfo = std::string(1, edge.vertex1Label) + " - " + std::string(1, edge.vertex2Label) + ": " + costStr;
        ...
        float sumOfMinimumCosts = 0.0f;
        for (float cost : minimumCosts) {
            sumOfMinimumCosts += cost;
        }
        glColor3f(1.0f, 1.0f, 1.0f); // Set color to white for text
        glRasterPos2f(-0.9f, -0.9f); // Set the position for the sum of minimum costs text
        std::string sumText = "Sum of Minimum Costs: " + std::to_string(sumOfMinimumCosts);
        for (const char& c : sumText) {
            glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, c);
        }
    ...
    if (vertices.empty()) {
        return false;  // No vertices in the graph, not connected.
    }
    ...
    while (!stack.empty()) {
        ...
        for (const auto& edge : edges) {
            if (edge.vertex1Label == vertices[current].label) {
                int neighborIndex = getIndexByLabel(edge.vertex2Label);
                if (!visited[neighborIndex]) {
                    visited[neighborIndex] = true;
                    stack.push(neighborIndex);
                }
            } else if (edge.vertex2Label == vertices[current].label) {
                int neighborIndex = getIndexByLabel(edge.vertex1Label);
                if (!visited[neighborIndex]) {
                    visited[neighborIndex] = true;
                    stack.push(neighborIndex);
                }
    ...
    for (bool v : visited) {
        if (!v) {
            return false;  // Not all vertices are reachable, not connected.
        }
    }

    return true;  // All vertices are reachable, the graph is connected.
}
    ...
    void primAlgorithm() {
        ...
        if (!isGraphConnected()) {
        std::cout << "Error: Graph is not connected. Cannot compute MST." << std::endl;
        return;
        }
            ...
            for (auto& edge : edges) {
                if (visited[getIndexByLabel(edge.vertex1Label)] != visited[getIndexByLabel(edge.vertex2Label)]) {
                    if (edge.cost < minCost) {
                        ...
                        minEdge = edge;
                        foundUnvisited = true;
                        minimumCosts.push_back(minCost);
                        edge.inMST = true;
